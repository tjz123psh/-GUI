...
- **CI 最后一步从未成功过（main 在本次推送之前就是红的）**：`Check whitespace errors` 先后暴露两个根因——① 依赖清单没有 `git`（`git: command not found`，exit 127，`454466f`/`f476d85` 两次运行因此失败而其余步骤全绿）；② 装上 git 后改为 `Not a git repository`：容器步骤以 root 运行、工作区属主是宿主机 uid，git 拒绝把检出目录当仓库（也可能是 checkout 走了 tarball 回退、根本没有 `.git`）。最终修法：依赖加 `git`、新增一步 `git config --global --add safe.directory "$GITHUB_WORKSPACE"`、checkout 设 `fetch-depth: 2`，并把强制检查改成**不依赖 git** 的检出内容扫描（全树文本文件行尾空白，比原来"只看新增行"更严且永远会执行），git 历史可用时再附加 `git diff --check HEAD^ HEAD`。
- **回归用例在 root 环境下失效**：`install_uninstall.sh` 里"用户可写 wrapper 必须被拒"的用例在 CI（容器以 root 运行）下前提不成立——新建文件天然 root-owned，守卫会正确放行。改为用 mode 0777 构造，组/其他可写这一半判据在 uid 0 与普通用户下触发同一句拒绝；已在本机与 `unshare -rU` 两种视角分别验证，正向对照（root-owned 0755 放行）同时确认。
